...
export function ProductForm({ product, filaments, printers, brands, onSuccess, onCancel }: ProductFormProps) {
  ...
  const form = useForm<z.infer<typeof ProductSchema>>({
    ...
    defaultValues: product || {
      ...
      pesoGramas: 0,
      imageUrl: "",
      custoModelagem: 0, // Default value
      custosExtras: 0,    // Default value
      margemLucroPercentual: 20, // Default value
    },
  });
  ...
  async function onSubmit(values: z.infer<typeof ProductSchema>) {
    try {
      const dataToSave: Product = {
        id: product?.id || '', // Handle if product.id is undefined for new products (though schema doesn't require it for DB)
        nome: values.nome,
      ...
      if (product && product.id) {
        ...
        // For create, remove 'id' if your action/DB handles ID generation
        const createData = { ...dataToSave };
        // delete createData.id; // Uncomment if ID is auto-generated by backend/action
